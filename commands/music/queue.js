...
module.exports = {
    name: 'queue',
    description: 'gives queue of the player',
    /**
     * 
     * @param {Client} client 
    ...
    async execute(interaction, client, error) {
        ...
        try {
            ...
            return interaction.reply({
                embeds: [new MessageEmbed().setColor('BLURPLE').setDescription(`${queue.songs.map((song, id) => `\n**${id + 1}** - ${song.name} - \`${song.formattedDuration}\``)}`)]
            }).then(setTimeout(() => setTimeout(() => interaction.deleteReply().catch(e => error.send({ embeds: [new MessageEmbed().setColor('RED').setDescription(`${e}`)] }))), 60000));
